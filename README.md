...
买入和卖出会使用彩色菱形标记区分：`🔸 BUY` 表示买入，`🔹 SELL` 表示卖出。每条提醒使用“标题 + 缩进明细”的格式，明细行用全角空格缩进，避免钉钉和 Discord 把它误解析成普通 Markdown 列表。Discord 和钉钉都支持 Markdown 的一部分，但它们的 Webhook Markdown 都不支持任意字体颜色，所以不能像 HTML 一样指定蓝色或红色字体。
